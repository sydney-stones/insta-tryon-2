...
/**
 * Standalone Khanums demo page — pre-made example for sales calls.
...
export const KHANUMS_PRODUCTS: Record<string, DemoProduct> = {
  'kodil-dress-sage-green': {
    id: 70,
    name: 'Kodil Dress - Sage Green',
    brand: 'KHANUMS',
    price: '£270.00',
    category: 'Dresses',
    sizes: ['XS', 'S', 'M', 'L', 'XL'],
    gallerySrcs: [
      '/result-images/khanums1.webp',
      '/result-images/khanums2.webp',
      '/result-images/khanums3.webp',
    ],
...
interface KhanumsDemoPageProps {
  productSlug: keyof typeof KHANUMS_PRODUCTS;
}

const KhanumsDemoPage: React.FC<KhanumsDemoPageProps> = ({ productSlug }) => {
  const product = KHANUMS_PRODUCTS[productSlug];
      ...
      <header style={{ borderBottom: '1px solid #e5e5e5', position: 'sticky', top: 0, backgroundColor: '#fff', zIndex: 40 }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '60px' }}>
          <nav style={{ display: 'flex', gap: '24px', fontSize: '11px', letterSpacing: '0.15em', fontWeight: 500 }}>
            {['NEW IN','DRESSES','SETS','TOPS'].map(n => (
...
export default KhanumsDemoPage;
