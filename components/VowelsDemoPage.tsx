...
export const VOWELS_PRODUCTS: Record<string, DemoProduct> = {
  'cezanne-zip-up-jacket': {
    id: 60,
    name: 'Cézanne Zip-Up Jacket',
    brand: 'VOWELS',
    price: '$795 USD',
    category: 'Jackets',
    sizes: ['XS', 'S', 'M', 'L', 'XL'],
    gallerySrcs: [
      '/result-images/vowels4.jpg',
      '/result-images/vowels5.jpg',
      '/result-images/vowels6.jpg',
    ],
    faceSrc: '/result-images/IMG_8175.jpeg',
    bodySrc: '/result-images/sydbody Background Removed.png',
    afterSrc: '/result-images/vowels4-nb2-2k-tryon.png',
    description: 'The Cézanne Zip-Up Jacket in a refined silhouette with clean zip closure. A considered piece built for longevity over trend.',
  },
  'overdyed-painter-pants': {
    id: 61,
    name: 'Overdyed Painter Pants',
    brand: 'VOWELS',
    price: '$395 USD',
    category: 'Trousers',
    sizes: ['XS', 'S', 'M', 'L', 'XL'],
    gallerySrcs: [
      '/result-images/vowels1.jpg',
      '/result-images/vowels2.jpg',
...
interface VowelsDemoPageProps {
  productSlug: keyof typeof VOWELS_PRODUCTS;
}

const VowelsDemoPage: React.FC<VowelsDemoPageProps> = ({ productSlug }) => {
  const product = VOWELS_PRODUCTS[productSlug];
      ...
      <header style={{ borderBottom: '1px solid #e5e5e5', position: 'sticky', top: 0, backgroundColor: '#fff', zIndex: 40 }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '60px' }}>
          <nav style={{ display: 'flex', gap: '24px', fontSize: '11px', letterSpacing: '0.15em', fontWeight: 500 }}>
            {['NEW','TOPS','BOTTOMS','OUTERWEAR'].map(n => (
              <span key={n} style={{ color: '#aaa' }}>{n}</span>
            ))}
          </nav>
          <span style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', fontSize: '20px', fontWeight: 300, letterSpacing: '0.12em', color: '#111' }}>
            VOWELS
          </span>
          <div />
        </div>
              ...
              {!isLoadingState(animState) && !isResultState(animState) && (
                <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '48px', borderBottom: '1px solid #D1D5DC', flexShrink: 0, backgroundColor: '#fff' }}>
                    <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', padding: '0px 16px', gap: '8px', height: '48px', borderRight: '1px solid #D1D5DC', minWidth: '113px' }}>
