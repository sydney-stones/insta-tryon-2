...
import { generateSingleProductTryOnNanoBanana } from '../services/geminiService';

interface SingleProductTryOnNanoBananaProps {
  onBack: () => void;
}

const SingleProductTryOnNanoBanana: React.FC<SingleProductTryOnNanoBananaProps> = ({ onBack }) => {
...
Return ONLY the final photorealistic virtual try-on image.`;
  ...
  const getResultFilename = (): string => {
    if (garmentImages.length === 0) return `nb2-tryon-${resolution.toLowerCase()}.png`;
    ...
    return `${baseName || firstName}-nb2-${resolution.toLowerCase()}-tryon.png`;
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateSingleProductTryOnNanoBanana(
        faceImage,
        bodyImage,
        garmentImages,
      ...
      console.error('Nano Banana 2 try-on error:', err);
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">Multi-angle garment reference with Gemini 3.1 Flash Image</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {!isProcessing && (
              <label className="mb-4 flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-purple-300 rounded-lg cursor-pointer bg-purple-50 hover:bg-purple-100">
                <div className="flex flex-col items-center justify-center">
                  ...
                  <p className="text-sm text-purple-700 font-semibold">Upload Product Images</p>
                  <p className="text-xs text-purple-600">Select multiple angles</p>
                </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="mb-6 space-y-3">
                ...
                <div className="space-y-2">
                  {(['1K', '2K', '4K'] as const).map((res) => (
                    <label
                      key={res}
                      className="flex items-start p-3 border-2 rounded-lg cursor-pointer transition-colors hover:bg-gray-50"
                      style={{ borderColor: resolution === res ? '#9333ea' : '#e5e7eb' }}
                    >
                      <input
                        type="radio"
                        name="nb2-resolution"
                      ...
                      <div className="flex-1">
                        <div className="font-medium text-sm">
                          {res} Resolution{res === '2K' ? ' (Recommended)' : ''}
                        </div>
                        <div className="text-xs text-gray-500 mt-1">
                          {res === '1K' && 'Faster generation, good for previews'}
                          {res === '2K' && 'Balanced quality and speed'}
                          {res === '4K' && 'Highest quality, slower generation'}
                        </div>
                      </div>
                    </label>
                  ))}
              ...
              <div className="mb-6 p-4 bg-purple-50 border border-purple-200 rounded-lg">
                <h3 className="text-sm font-semibold text-purple-900 mb-2">Nano Banana 2 Test Mode</h3>
                <ul className="text-xs text-purple-800 space-y-1">
                  <li>Running on Gemini 3.1 Flash Image</li>
                  <li>Upload multiple angles of ONE product</li>
                  <li>Front, back, model, and detail images improve accuracy</li>
                  <li>Compare results against Gemini 3 Pro tool</li>
                </ul>
              </div>
              ...
              {isProcessing && (
                <div className="flex flex-col items-center justify-center py-8">
                  ...
                  <p className="text-sm text-gray-600">Generating with Nano Banana 2...</p>
        ...
        <div className="mt-8 bg-purple-50 border border-purple-200 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-purple-900 mb-3">Nano Banana 2 — Single Product Tips</h3>
          <ul className="list-disc list-inside space-y-2 text-sm text-purple-800">
            ...
            <li>Download filename includes <strong>nb2</strong> to distinguish from Gemini 3 Pro outputs</li>
          </ul>
        </div>
      </div>
    </div>
...
export default SingleProductTryOnNanoBanana;
