...
Return ONLY the final photorealistic virtual try-on image showing this person wearing the garment.`;
  ...
  const handleBatchGeneration = async () => {
    ...
    if (processingMode === 'sequential') {
      ...
      // Bulk/parallel processing - all at once
      const promises = garmentImages.map(async (garment, index) => {
        // Update status to processing
        setBatchResults(prev => prev.map((result, idx) =>
        ...
        try {
          ...
          // Update with completed result
          setBatchResults(prev => prev.map((res, idx) =>
            idx === index ? { ...res, status: 'completed', imageUrl: result } : res
          ));
          setCompletedCount(prev => prev + 1);

          return { index, success: true };
        } catch (err) {
          console.error(`Error processing garment ${index}:`, err);
  ...
  const handleDownloadAsZip = async () => {
    ...
    try {
      const zip = new JSZip();
      const folder = zip.folder('virtual-tryon-results');
      ...
      for (const result of completedResults) {
        ...
        // Add to zip with garment name
        folder?.file(`${result.garmentName}-tryon-${resolution}.png`, blob);
      }
      ...
      link.download = `virtual-tryon-batch-${resolution}-${Date.now()}.zip`;
      link.click();
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="mb-6 space-y-3">
                ...
                <div className="space-y-2">
                  <label className="flex items-start p-3 border-2 rounded-lg cursor-pointer transition-colors hover:bg-gray-50 relative"
                    style={{ borderColor: processingMode === 'sequential' ? '#9333ea' : '#e5e7eb' }}>
                    <input
                      type="radio"
                      name="processingMode"
                      value="sequential"
                      checked={processingMode === 'sequential'}
                      onChange={(e) => setProcessingMode(e.target.value as 'sequential' | 'bulk')}
                      className="mt-1 mr-3"
                      disabled={isBatchProcessing}
                    />
                    <div className="flex-1">
                      <div className="font-medium text-sm">Sequential Processing</div>
                      <div className="text-xs text-gray-500 mt-1">One by one - safer, progress tracking</div>
                    </div>
                  </label>
                  <label className="flex items-start p-3 border-2 rounded-lg cursor-pointer transition-colors hover:bg-gray-50 relative"
                    style={{ borderColor: processingMode === 'bulk' ? '#9333ea' : '#e5e7eb' }}>
                    <input
                      type="radio"
                      name="processingMode"
                      value="bulk"
                    ...
                    />
                    <div className="flex-1">
                      <div className="font-medium text-sm">Bulk Create (Parallel)</div>
                      <div className="text-xs text-gray-500 mt-1">All at once - faster but uses more resources</div>
                    </div>
                  </label>
                </div>
              </div>
                ...
                <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-blue-900">
                      {processingMode === 'sequential' ? 'Processing Sequentially' : 'Bulk Creating (Parallel)'}
                    </span>
                  ...
                  {processingMode === 'sequential' && currentProcessingIndex >= 0 && (
                  ...
                  {processingMode === 'bulk' && (
                    <p className="text-xs text-blue-700 mt-2">
                      Processing all {garmentImages.length} garments in parallel...
                    </p>
                  )}
                </div>
              )}
